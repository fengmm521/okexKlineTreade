...
basecount = 1
savepth = 'data/simulation135.txt'
...
class TradeType(object):
    ...
    def __init__(self,spth = savepth):
...
def testIsState(dat1,dat3,dat5):
    ...
    if macd1type == 1:
        tradetool.isMacdJXs[0] = True
        tradetool.isMacdSXs[0] = False
    elif macd1type == -1:
        tradetool.isMacdJXs[0] = False
        tradetool.isMacdSXs[0] = True

    if macd3type == 1:
        tradetool.isMacdJXs[1] = True
        tradetool.isMacdSXs[1] = False
    elif macd3type == -1:
        tradetool.isMacdJXs[1] = False
        tradetool.isMacdSXs[1] = True

    if macd5type == 1:
        tradetool.isMacdJXs[2] = True
        tradetool.isMacdSXs[2] = False
    elif macd5type == -1:
        tradetool.isMacdJXs[2] = False
        tradetool.isMacdSXs[2] = True

    if tradetool.isOpenLong:
        if (tradetool.isMacdSXs[0] and (macdchange3 < 0 or macdchange5 < 0)):
            tradetool.closeLong('1')
        elif tradetool.isMacdSXs[1]:
            tradetool.closeLong('2')
        elif tradetool.isMacdSXs[2]:
            tradetool.closeLong('3')
    else:
        if tradetool.isMacdJXs[0] and macdv3 > 0 and macdv5 > 0 and macdchange3 > 0:
            tradetool.openLong()

    if tradetool.isOpenShort:
        if (tradetool.isMacdJXs[0] and (macdchange3 > 0 or macdchange5 > 0)):
            tradetool.closeShort('1')
        elif tradetool.isMacdJXs[1]:
            tradetool.closeShort('2')
        elif tradetool.isMacdJXs[2]:
            tradetool.closeShort('3')
    else:
        if tradetool.isMacdSXs[0] and macdv3 < 0 and macdv5 < 0 and macdchange3 < 0:
            tradetool.openShort()

trademacdtool = TradeType('data/simulationmacd135.txt')
